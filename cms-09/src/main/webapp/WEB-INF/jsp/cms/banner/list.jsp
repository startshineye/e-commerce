...
<div class="ti">轮播图查询</div>
 	<table class="SearchTab">
			<tr>
				...
				<td><form:select path="status_c" items="${statusMap}" class = "text-m"/></td>
				<td align="right">顺序</td>
				<td><form:input path="order_num_c" class = "text-m"/></td>
				<td align="right">终端类型</td>
				<td><form:select path="type_c" items="${typeMap}" class = "text-m" /></td>
			</tr>
			<tr>
				<td></td>
				<td></td>
	...
	<!-- 数据显示 -->
			<c:forEach items="${dataList}" var="wz" varStatus="vs">
				<tr>
					...
					<td class="td-del" onclick="toAction('${webPath}/banner/delete?ids=${wz.id}');"></td>
...
<script type="text/javascript">
	...
	//删除多选的checkBox 
	function to_delete(ids){
		if(checkDel(ids)){
			toAction('${webPath}/banner/delete?ids='+ids);
		}
	}
</script>
</html>